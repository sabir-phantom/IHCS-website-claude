...
const OUT = 'D:/Sabir/Work/IC/iqbal-catering-v2';
...
<div class="jharokha">
  ...
  ${video
    ? `<video data-ambient data-src="${r}assets/video/${video}" muted loop playsinline preload="none" poster="${r}assets/img/${img}" aria-hidden="true" tabindex="-1"></video>`
    : `<img src="${r}assets/img/${img}" alt="${alt}" width="${w}" height="${h}"${eager ? '' : ' loading="lazy"'}>`}
...
const svcCards = r => `<ul class="svc-cards" data-stagger>${SERVICES.map(s => `
  <li><a class="svc-card" href="${r}${s.slug}/">
    <div class="pic"><img src="${r}assets/img/${s.card}" alt="${s.cardAlt}" loading="lazy"></div>
...
<section class="deck-sec" id="dawats" aria-label="Menu cards from real events">
  <div class="wrap">
    ...
    <div class="deck" id="deck">
      <button class="mcard" type="button"><img src="${r}assets/img/gallery-marigold-menu.jpg" alt="A wedding menu card among marigolds, with an Iqbal Catering table card" loading="lazy" width="1152" height="2048"></button>
      <button class="mcard" type="button"><img src="${r}assets/img/gallery-menu-floral.jpg" alt="A floral wedding menu card on a reception table" loading="lazy" width="1152" height="2048"></button>
      <button class="mcard" type="button"><img src="${r}assets/img/gallery-mayeesha-nabil-menu.jpg" alt="The printed menu card from Mayeesha and Nabil's wedding" loading="lazy" width="1152" height="2048"></button>
      <button class="mcard" type="button"><img src="${r}assets/img/gallery-starter-menu.jpg" alt="A framed starter and main course menu board from a catered event" loading="lazy" width="1152" height="2048"></button>
      <button class="mcard" type="button"><img src="${r}assets/img/gallery-menu-display.jpg" alt="A menu display with an Iqbal Catering branded table card" loading="lazy" width="1152" height="2048"></button>
      <button class="mcard" type="button"><img src="${r}assets/img/gallery-menu-tissuebox.jpg" alt="A wedding menu beside an Iqbal Hossain Catering Service tissue box" loading="lazy" width="1152" height="2048"></button>
    </div>
...
<div class="reel" style="background-image:url('${r}assets/img/${poster}')">
  <video data-src="${r}assets/video/${file}" playsinline preload="none" poster="${r}assets/img/${poster}"></video>
...
<div class="menus">
  ...
  <div class="paper" id="menuPanel" role="tabpanel" data-io>
    <div class="paper-top">
      <img src="${r}assets/img/logo.png" alt="" width="64" height="64" loading="lazy">
    ...
    <div class="foot-top">
      <div>
        <div class="reveal-logo">
          <video data-ambient data-once data-alpha data-src="${r}assets/video/logo-reveal.webm" muted playsinline preload="none" poster="${r}assets/img/logo-reveal-poster.png" width="480" height="496" aria-hidden="true" tabindex="-1"></video>
        </div>
...
${footer(r)}
${home ? `<script src="assets/js/hero.js" defer></script>\n` : ''}<script src="${r}assets/js/site.js" defer></script>
</body>
</html>
`;
...
<section aria-label="Our founder">
  <div class="wrap split rev">
    <div class="media" data-io>${jharokha(r, '', '', {}).replace(`src="${r}assets/img/"`, `src="${r}assets/team/founder.jpg"`).replace('alt=""', 'alt="Mohd. Iqbal Hossain, founder of Iqbal Catering"')}</div>
    <div class="copy" data-io>
...
<section aria-label="Managing partners">
  <div class="wrap">
    ...
    <ul class="partners" data-stagger>${PARTNERS.slice(0, 4).map(p => `<li class="partner">${jharokha(r, '', p.name, { w: p.w, h: p.h }).replace(`src="${r}assets/img/"`, `src="${r}assets/team/${p.img}"`)}<b>${p.name}</b><span>${p.role}</span><span>${p.focus}</span></li>`).join('')}</ul>
  </div>
</section>
<div class="arcade" aria-hidden="true"></div>
<section class="marble" aria-label="The second generation">
  <div class="wrap split rev">
    <div class="media" data-io>${jharokha(r, '', 'Abdul Qayyuum', { w: 1200, h: 1800, tag: 'Mr. Abdul Qayyuum' }).replace(`src="${r}assets/img/"`, `src="${r}assets/team/abdul-qayyuum.jpg"`)}</div>
    <div class="copy" data-io>
...
<section class="marble" id="photos" aria-label="Photo gallery">
  <div class="wrap">
    <div class="gallery">${GALLERY.map(([f, w, h, c]) => `<button type="button" data-caption="${c.replace(/"/g, '&quot;')}" aria-label="View photo: ${c.replace(/"/g, '&quot;')}"><img src="${r}assets/img/${f}" alt="${c.replace(/"/g, '&quot;')}" width="${w}" height="${h}" loading="lazy"><figcaption aria-hidden="true">${c}</figcaption></button>`).join('')}</div>
  </div>
</section>
<div class="arcade to-emerald" aria-hidden="true"></div>
${reels(r)}
...
<section aria-label="Complete contact information">
  <div class="wrap">
    ...
    <ul class="partners five" data-stagger>${PARTNERS.map(p => `<li class="partner">${jharokha(r, '', p.name, { w: p.w, h: p.h }).replace(`src="${r}assets/img/"`, `src="${r}assets/team/${p.img}"`)}<b>${p.name}</b><span>${p.role}</span><a class="phone" href="tel:${p.phone}">${p.phoneLabel}</a></li>`).join('')}</ul>
